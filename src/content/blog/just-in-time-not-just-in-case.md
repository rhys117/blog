...
Add one of those actions by hand and skip the include, and you might have diverged from my conventions I've setup and 
instead fallen back to its training data.
...
No bad commit to walk back, or another turn of instructions to the LLM because we'd course corrected early.
...
It works through sentinel files that reset each session, and each working session gets its own set of sentinel files.
...
The difference is returning 389 lines, compared to 26 lines, using the domain query. Whilst most of it is call sites 
have nothing to do with whatever I'm actually changing. So it lets the discovery phase run in a much more targeted way, 
and hone in on the files and nodes it really needs.
